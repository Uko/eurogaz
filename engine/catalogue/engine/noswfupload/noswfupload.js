...
var $ = {
     ...
     * @param css	- String	- main noswfupload css file (the wrap)
     ...
     * @param wrap	- Object	- generated wrap object
     ...
     * @param e		- void		- generated event or undefined
     * @return      Boolean     false
     */
    ...
    sendFile:(function(toString){
		/**
		 *	@description make a HTTP header to send file and some parameters
		 *	@param	String		boundary string
		 *	@param	String		name of field(parameter) to store file
		 *	@param	Object		file object from handler
		 *	@param	Object		list of parameters param: value
		 */
        var multipart = function(boundary, name, file, paramList)
			{
				var header = "";
				if(paramList)
					for(param in paramList)
					{
						header += "".concat(
							"--", boundary, CRLF,
							'Content-Disposition: form-data; name="', param, '"', CRLF,
							CRLF,
							paramList[param], CRLF,
							"--", boundary, CRLF);
					}
				else
					header = "".concat("--", boundary, CRLF);
                header += "".concat(
        ...
        if(xhr.upload || xhr.sendAsBinary)
            sendFile = function(handler, maxSize)
			{
				...
				{
					parms += param+'='+handler.parameters[param]+'&';
				}
//alert("handler.url:"+handler.url+'?'+parms.substr(0, parms.length-1));
                xhr.open("post", handler.url+'?'+parms.substr(0, parms.length-1), true);
                if(!xhr.upload){
...
//alert(handler.file.getAsBinary);
                if(handler.file.getAsBinary)
				{
                    var boundary = "AjaxUploadBoundary" + (new Date).getTime();
//alert(multipart(boundary, handler.name, handler.file));
                    xhr.setRequestHeader("Content-Type", "multipart/form-data; boundary=" + boundary);
                    xhr[xhr.sendAsBinary ? "sendAsBinary" : "send"](multipart(boundary, handler.name, handler.file));
                } else
				{
//alert("handler.file:"+handler.file);
//var debugInfo;
//for(field in handler.file)
//	debugInfo += "\n" + field + "=" + handler.file[field];
//alert(debugInfo);
//alert(	"size:"+handler.file.size+
//		"\ntype:"+handler.file.type);
//				var tmpHeader;
//				for(param in handler.parameters)
//				{
//					tmpHeader += "".concat(
...
//				var boundary = "AjaxUploadBoundary" + (new Date).getTime();
        ...
        // Internet Explorer, Opera, others
            ...
            sendFile = function(handler, maxSize)
			{
//alert("Internet Explorer, Opera, others");
				var params = "";
				for(param in handler.parameters)
				{
					params += param+'='+handler.parameters[param]+'&';
				}
...
//alert(handler.url+'?'+parms.substr(0, parms.length-1));
                form.setAttribute("action", handler.url.concat(-1 === handler.url.indexOf("?") ? "?" : "&", params.substr(0, params.length-1)));
     ...
     * @param handler	- Object	- handler used to perform operations (for example, a wrap Object)
     * @param maxSize	- Number	- optional file maxSize
     * @return			- Object	- the handler itself
     */
     ...
     * @param input			- DOMElement	- the input type file to wrap to obtain a noswfupload component.
     * @param maxSize		- Number		- optional maxSize for each selected file
	 * @param parameters	- JSONstring	- additional parameters to send with HTTP request
     */
    wrap:function(input, maxSize, parameters)
	{
        ...
        return  $.events({
			...
			parameters: window.json_parse(parameters),
...
//alert("this.parameters:"+this.parameters.namePrefix);
            ...
            show:function(total, current, totalProp, currentProp){
                ...
                //style[currentProp || "height"] = (current >> 0) + "px";
				cstyle["height"] = tstyle["height"] = "10px";
                tstyle[totalProp || "width"] = (total >> 0) + "px";
                cstyle[currentProp || "width"] = (current >> 0) + "px";
            }
        });
    }
};
